...
def apply_patch():
    ...
    to_remove = """                    // Add URL Summary if links exist
                    if (analysis.links.isNotEmpty()) {
                        Spacer(modifier = Modifier.height(16.dp))
                        androidx.compose.material3.HorizontalDivider(color = cardBorder.copy(alpha = 0.5f), thickness = 1.dp)
                        ...
                        if (totalUrls == 1) {
                            ...
                            Text(text = linkStatus, fontSize = 14.sp, fontWeight = FontWeight.Bold, color = if (knownThreats > 0) androidx.compose.ui.graphics.Color(0xFFE53935) else textPrimary)
                        } else if (totalUrls > 1) {
                            ...
                            Text(text = summaryStr, fontSize = 14.sp, fontWeight = FontWeight.Bold, color = if (knownThreats > 0) androidx.compose.ui.graphics.Color(0xFFE53935) else textPrimary)
                        }
                    }"""
    ...
    if to_remove in content:
        ...
        print("Could not find old summary")
